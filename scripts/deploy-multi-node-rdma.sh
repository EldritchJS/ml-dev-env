...
# Deploy multi-node StatefulSet for distributed training (RDMA/RoCE mode)
# Usage: ./scripts/deploy-multi-node-rdma.sh [namespace]
...
cat k8s/statefulset-multi-node-rdma.yaml | sed "s|namespace: nccl-test|namespace: $NAMESPACE|g" | oc apply -f -

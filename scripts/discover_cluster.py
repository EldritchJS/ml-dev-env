...
class ClusterDiscovery:
    ...
    def discover_cluster_info(self) -> dict[str, str]:
        ...
        self._run_command(["oc", "version", "-o", "json"], check=False)

        return {"api": api, "namespace": self.namespace}
    ...
    def discover_gpu_nodes(self) -> dict[str, Any]:
        ...
        output = self._run_command(
            ["oc", "get", "nodes", "-l", "nvidia.com/gpu.present=true", "-o", "json"], check=False
        )

        if not output:
            print("   ⚠️  No GPU nodes found with label nvidia.com/gpu.present=true")
            return {"nodes": [], "gpu_type": "Unknown", "gpus_per_node": 0}
        ...
        for node in nodes_data.get("items", []):
            ...
            if gpu_count and int(gpu_count) > 0:
                ...
                for key, value in labels.items():
                    if "gpu" in key.lower() and (
                        "h100" in value.lower() or "a100" in value.lower()
                    ):
                        gpu_type = value
                        break
                ...
                if gpu_type == "Unknown":
                    ...
                    node_desc = self._run_command(
                        ["oc", "describe", "node", node_name], check=False
                    )
    ...
    def discover_rdma(self, gpu_nodes: list[str]) -> dict[str, Any]:
        ...
        net_attach = self._run_command(
            ["oc", "get", "network-attachment-definitions", "-A", "-o", "json"], check=False
        )
        ...
        node_info = self._run_command(["oc", "get", "node", node, "-o", "json"], check=False)
        ...
        if mlx_devices or rdma_networks:
            devices_str = (
                ",".join(sorted(set(mlx_devices)))
                if mlx_devices
                else "mlx5_6,mlx5_7,mlx5_10,mlx5_11"
            )
            ...
            return {"enabled": False, "devices": "", "reason": "No InfiniBand devices detected"}

    def discover_storage(self) -> dict[str, Any]:
        """Discover storage classes"""
        print("💾 Discovering storage classes...")

        output = self._run_command(["oc", "get", "storageclass", "-o", "json"], check=False)

        if not output:
            print("   ⚠️  No storage classes found")
            return {"class_rwx": "nfs-csi", "class_rwo": "ceph-rbd", "mode": "volumeClaimTemplates"}
        ...
        if rwx_classes:
            ...
            print("   ℹ️  No RWX storage found, using per-pod storage")

        print(f"   ✓ RWO storage: {class_rwo}")

        return {"class_rwx": class_rwx, "class_rwo": class_rwo, "mode": mode}
    ...
    def discover_security(self) -> dict[str, Any]:
        ...
        scc_output = self._run_command(
            ["oc", "get", "scc", "privileged", "-o", "json"], check=False
        )
    ...
    def generate_config(self, cluster_name: str) -> dict[str, Any]:
        ...
        config = {
            ...
            "nodes": {"gpu_nodes": gpu_info["nodes"]},
            "storage": {
            ...
            "network": {
                ...
                "tcp": {"interface_exclude": "^lo,docker0", "p2p_level": "NVL"},
            },
    ...
    def _generate_notes(self, cluster_info, gpu_info, rdma_info, storage_info) -> str:
        ...
        notes = [
            "Auto-discovered cluster configuration",
            "",
            "Cluster details:",
        ...
        if rdma_info["enabled"]:
            notes.extend(
                [
                    "",
                    "2. Grant privileged SCC (required for RDMA):",
        ...
        if storage_info["mode"] == "rwx":
            notes.extend(
                [
                    "",
                    "3. Verify NFS/RWX storage is available:",
                    "   oc get pods -n nfs",
                ]
            )
...
def main():
    ...
    parser.add_argument("--name", required=True, help="Cluster name for the configuration file")
    parser.add_argument(
        "--namespace", help="Kubernetes namespace to use (default: current namespace)"
    )
    parser.add_argument("--output", help="Output file path (default: clusters/<name>.yaml)")
    parser.add_argument(
        "--dry-run", action="store_true", help="Print configuration to stdout instead of saving"
    )
    ...
    header += "# Auto-generated by discover-cluster.py\n"
    ...
    if args.dry_run:
        ...
        print(
            f"3. Deploy: make deploy-cluster CLUSTER={args.name} MODE={'rdma' if config['network']['rdma']['enabled'] else 'tcp'}"
        )

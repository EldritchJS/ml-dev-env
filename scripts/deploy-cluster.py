...
def generate_statefulset(config: Dict[str, Any], mode: str, output_file: str):
    ...
    if config['security'].get('service_account'):
        service_account_line = f"      serviceAccountName: {config['security']['service_account']}\n"
        content = content.replace('      restartPolicy: Always\n', 
...
def print_setup_instructions(config: Dict[str, Any]):
    ...
    if config['security'].get('requires_privileged_scc'):
        ...
        print(f"   oc adm policy add-scc-to-user privileged -z {config['security']['service_account']} -n {config['cluster']['namespace']}")
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true',
                       help='Generate configs but do not apply')
